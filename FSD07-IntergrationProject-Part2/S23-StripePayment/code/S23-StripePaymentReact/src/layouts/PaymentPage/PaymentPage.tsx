...
export const PaymentPage = () => {
  // Retrieve authentication state using Okta
  ...
  // Effect to fetch payment fees when the authentication state changes
  useEffect(() => {
  ...
  // Initialize Stripe elements and Stripe instance
      ...
      .then(async function (result: any) {
        if (result.error) {
          ...
          const requestOptions = {
            method: "PUT",
            headers: {
              Authorization: `Bear ${authState?.accessToken?.accessToken}`,
              "Content-Type": "application/json",
            },
          };
          const stripeResponse = await fetch(url, requestOptions);
  ...
  // Render UI based on payment state
  if (loadingFees) {
